...
public final class ModelComparator implements ICompareInputDetailsProvider {
	...
	protected Comparison comparisonResult;
	 ...
	 */
	public Comparison compare(CompareConfiguration configuration) {
		if (!loadingSucceeded) {
			// We couldn't load the resource. It's useless to carry on.
			comparisonResult = CompareFactory.eINSTANCE.createComparison();
		}
	 ...
	 * @return Result of the comparison of the loaded resources.
	 */
	public Comparison compareSilentlyInThreadUI(CompareConfiguration configuration) {
		if (!loadingSucceeded) {
			// We couldn't load the resource. It's useless to carry on.
			comparisonResult = CompareFactory.eINSTANCE.createComparison();
		}
	...
	public void dispose() {
		final Thread longRunningDispose = new Thread() {
			@Override
			public void run() {
				final Comparison snapshot = getComparisonResult();
	 ...
	 */
	protected Comparison doResourceCompare() {
	...
	public class RunnableResourceCompare implements IRunnableWithProgress {
		 ...
		 */
		public Comparison getComparisonResourceSnapshot() {
			return snapshot;
		}
	 ...
	 *             exception
	 */
	protected Comparison doResourceCompareInThreadUI(IProgressMonitor monitor) throws InterruptedException {
		return EMFCompare.compare(leftResource, rightResource);
	}
	 ...
	 */
	protected Comparison doResourceSetCompare() {
	...
	public class RunnableResourceSetCompare implements IRunnableWithProgress {
		 ...
		 * @return ComparisonResourceSetSnapshot
		 */
		public Comparison getComparisonResourceSetSnapshot() {
			return snapshot;
		}
	 ...
	 *             exception
	 */
	protected Comparison doResourceSetCompareInThreadUI(IProgressMonitor monitor) throws InterruptedException {
		// create snapshot
		return EMFCompare.compare(leftResource, rightResource);
	}
	 ...
	 *         resources.
	 */
	public Comparison getComparisonResult() {
		return comparisonResult;
	}
	...
	public boolean loadResources(ICompareInput input) {
		if (ancestorElement != input.getAncestor() || leftElement != input.getLeft()
			...
			try {
				...
				if (leftElement instanceof ResourceNode) {
					leftResource = loadLocal(((ResourceNode)leftElement).getResource());
				}
				if (rightElement instanceof ResourceNode) {
					rightResource = loadLocal(((ResourceNode)rightElement).getResource());
				}
	...
	private Resource loadLocal(IResource resource) throws IOException {
		URI uri = URI.createPlatformResourceURI(resource.getFullPath().toOSString(), true);
		Resource result = Resource.Factory.Registry.INSTANCE.getFactory(uri).createResource(uri);
	 ...
	 */
	public void setComparisonResult(Comparison snapshot) {
